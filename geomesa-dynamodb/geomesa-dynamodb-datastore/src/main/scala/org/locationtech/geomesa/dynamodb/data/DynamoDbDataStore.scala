...
class DynamoDbDataStore(val client: AmazonDynamoDB, config: GeoMesaDataStoreConfig) extends GeoMesaDataStore[DynamoDbDataStore](config) with LocalLocking {
...
/*
class DynamoDbDataStore(val session: Session, config: CassandraDataStoreConfig)
